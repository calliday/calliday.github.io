...
it checks **The Odyssey** at **Sandy at Jordan Commons**.

Not wired into this site — it's a standalone script meant to be copied out
and run elsewhere (e.g. as an n8n step, cron job, etc.).
...
direct `curl`/`urllib` requests with a normal browser `User-Agent`. Note it
does **not** send CORS headers, so it can only be called server-side (e.g.
from this script), not from a browser on a different origin.
...
```bash
# Check The Odyssey at Jordan Commons for open luxury seats over the next 5 days (default)
python3 check_luxury_seating.py

# Shorter window
python3 check_luxury_seating.py --days 3

# Check a different movie or location
python3 check_luxury_seating.py --film "Super Troopers 3"
